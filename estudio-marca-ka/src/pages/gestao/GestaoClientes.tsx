...
import type { Cliente, Usuario, PagamentoContrato, Socio } from '../../lib/database.types'
import {
  criarCliente,
  listarClientes,
    ...
    <>
      ...
      <div className="card">
        ...
        <form onSubmit={(e) => void salvar(e)}>
          ...
          <SociosContrato lista={f.socios ?? []} onChange={(l) => campo('socios', l)} />
...
// ---------------------------------------------------------------------------
// Sócios da empresa: lista com nome/CPF/RG/e-mail e "assina o contrato".
// ---------------------------------------------------------------------------
function SociosContrato({ lista, onChange }: { lista: Socio[]; onChange: (l: Socio[]) => void }) {
    ...
    <div>
      {lista.length === 0 && (
        <p style={{ fontSize: '0.82rem', color: '#837b6c', margin: '0 0 0.6rem' }}>
          Nenhum sócio adicionado (deixe assim se a empresa tem só o responsável acima).
        </p>
      )}
      {lista.map((s, i) => (
        <div
