...
export const FORMAS = [
  {
    ...
    id: 'shape-blob1',
    label: 'Forma 1',
    // proporção nativa do SVG (largura/altura) — usada para não distorcer a forma.
    ratio: 732 / 817,
  ...
  {
    // Forma quadrada (com cantinhos levemente arredondados) — opção reta, a
    // pedido da KA, além das 3 orgânicas.
    id: 'shape-quadrado',
    label: 'Quadrada',
    ratio: 1,
    d: 'M0.06,0 H0.94 C0.973,0 1,0.027 1,0.06 V0.94 C1,0.973 0.973,1 0.94,1 H0.06 C0.027,1 0,0.973 0,0.94 V0.06 C0,0.027 0.027,0 0.06,0 Z',
  },
]

export const FORMA_PADRAO = FORMAS[0].id

// Proporção (largura/altura) da forma escolhida — para dimensionar a caixa da foto
// sem esticar o clip-path (que usa objectBoundingBox e se ajusta à caixa).
export function ratioForma(id: string): number {
  return FORMAS.find((f) => f.id === id)?.ratio ?? 1
}

// Maior caixa com a proporção `ratio` que cabe dentro do espaço disponível (contain).
export function caixaContida(
  dispLargura: number,
  dispAltura: number,
  ratio: number,
): { largura: number; altura: number } {
  let largura = dispLargura
  let altura = largura / ratio
  if (altura > dispAltura) {
    altura = dispAltura
